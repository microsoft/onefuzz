    ...
    async fn test_radamsa_linux() -> anyhow::Result<()> {
        ...
        let config = Config {
            ...
            readonly_inputs: vec![SyncedDir {
                ...
                url: Some(BlobContainerUrl::parse(Url::from_directory_path(inputs).unwrap())?),
            }],
            crashes: SyncedDir {
                path: crashes_local,
                url: Some(BlobContainerUrl::parse(Url::from_directory_path(crashes).unwrap())?),
            },
            tools: Some(SyncedDir {
                path: tools_local,
                url: Some(BlobContainerUrl::parse(Url::from_directory_path(radamsa_dir).unwrap())?),
            }),
