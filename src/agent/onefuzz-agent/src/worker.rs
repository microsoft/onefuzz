...
impl Worker {
    pub fn new(setup_dir: impl AsRef<Path>, extra_dir: Option<impl AsRef<Path>>, work: WorkUnit) -> Self {
        let ctx = Ready {
...
impl State<Ready> {
    pub async fn run(self, runner: &mut dyn IWorkerRunner) -> Result<State<Running>> {
        let child = runner.run(&self.ctx.setup_dir, self.ctx.extra_dir.into(), &self.work).await?;
...
pub trait IWorkerRunner: Downcast {
    async fn run(&self, setup_dir: &Path, extra_dir: Option<PathBuf> , work: &WorkUnit) -> Result<Box<dyn IWorkerChild>>;
}
