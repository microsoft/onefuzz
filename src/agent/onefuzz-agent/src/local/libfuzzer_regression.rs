...
use crate::{
    local::common::{
        ...
        UiEvent, SyncCountDirMonitor, CHECK_FUZZER_HELP, CHECK_RETRY_COUNT, COVERAGE_DIR, CRASHES_DIR, NO_REPRO_DIR,
...
pub fn build_regression_config(
    ...
    let crashes = get_synced_dir(CRASHES_DIR, common.job_id, common.task_id, args)?.monitor_count(&event_sender)?;
    let regression_reports =
        get_synced_dir(REGRESSION_REPORTS_DIR, common.job_id, common.task_id, args)?.monitor_count(&event_sender)?;
    let check_retry_count = value_t!(args, CHECK_RETRY_COUNT, u64)?;

    let reports = get_synced_dir(REPORTS_DIR, common.job_id, common.task_id, args).ok().monitor_count(&event_sender)?;
    let no_repro = get_synced_dir(NO_REPRO_DIR, common.job_id, common.task_id, args).ok().monitor_count(&event_sender)?;
    let unique_reports =
        get_synced_dir(UNIQUE_REPORTS_DIR, common.job_id, common.task_id, args).ok().monitor_count(&event_sender)?;
