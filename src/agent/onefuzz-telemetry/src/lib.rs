...
pub fn track_metric(metric: &Metric, value: f64, properties: &[MetricData]) {
    use appinsights::telemetry::Telemetry;
    ...
    // try_broadcast_event(chrono::Utc::now(), metric, properties);
}
...
macro_rules! log_metrics {
    ($name: expr; $value: expr; $metrics: expr) => {{
        onefuzz_telemetry::track_metric(&$name, &$metrics, &$value);
        log::info!(
            "{} {}",
            $name.as_str(),
            onefuzz_telemetry::format_events(&$metrics)
        );
    }};
}
