...
Start-Process -wait -FilePath $exe -ArgumentList "-n -L -R $dest"
write-host "launching install packages"
Start-Process -wait -FilePath $exe -ArgumentList "-n -L -R $dest -P _autorebase,alternatives,base-cygwin,base-files,bash,binutils,bzip2,ca-certificates,coreutils,`
...
Start-Process -wait -FilePath $exe -ArgumentList "-n -L -R $dest -P wget,curl"
Start-Process -wait -FilePath $exe -ArgumentList "-n -L -R $dest -P cygwin-devel"
