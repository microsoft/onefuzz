    ...
    public async Async.Task ReimageNodes(Scaleset scaleset, IEnumerable<Node> nodes, NodeDisposalStrategy disposalStrategy) {
        ...
        switch (disposalStrategy) {
            case NodeDisposalStrategy.Decommission:
                _log.Info($"Skipping reimage of nodes in scaleset: {scaleset.ScalesetId:Tag:ScalesetId}, deleting nodes: {string.Join(", ", nodesToReimage.Select(n => (n.InstanceId, n.MachineId))):Tag:(InstanceIds, MachineIds)}");
