...
public class WebhookMessageLogOperations : Orm<WebhookMessageLog>, IWebhookMessageLogOperations
{
    ...
    public WebhookMessageLogOperations(IStorage storage, IQueue queue, IWebhookMessageLogOperations webhookMessage, ILogTracerFactory loggerFactory) : base(storage)
    {
        _queue = queue;
    ...
    public void process()
    {
        if (_webhookMessage.State == WebhookMessageState.Failed || _webhookMessage.State == WebhookMessageState.Succeeded)
        {
            log.AddTags(
                    new[] {
                        ("WebhookId", _webhookMessage.WebhookId.ToString()),
                        ("EventId", _webhookMessage.EventId.ToString()) }
                    ).
                Error($"webhook message already handled. {_webhookMessage.WebhookId}:{_webhookMessage.EventId}");  
        }

        _webhookMessage.TryCount++;
        
        log.info($"sending webhook: {_webhookMessage.WebhookId}:{_webhookMessage.EventId}")

    }


    public async Task ProcessFromQueue(WebhookMessageQueueObj webhookMessage)
    {
        message = await _webhookMessage.get(webhookMessage.WebhookId, webhookMessage.EventId);
        
        if (message == null)
        {
            log.AddTags(
                new[] {
                    ("WebhookId", webhookMessage.WebhookId.ToString()),
                        ("EventId", webhookMessage.EventId.ToString()) }
                    ).
                Error($"Webhook with webhookId: {webhookMessage.WebhookId} and eventId: {webhookMessage.EventId} not found.");
            )
        }
