...
namespace ApiService.TestHooks {
    ...
    public class EventsTestHooks {
        ...
        public EventsTestHooks(ILogger<EventsTestHooks> log, IConfigOperations configOps, IEvents events) {
            _log = log;
            _log.AddTag("TestHooks", nameof(EventsTestHooks));
        ...
        public async Task<HttpResponseData> SendEvent([HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "testhooks/events/sendEvent")] HttpRequestData req) {
            _log.LogInformation("Send event");
